...
must_haves:
  ...
  key_links:
    ...
    - from: "evcc-smartload/rootfs/app/main.py"
      to: "evcc-smartload/rootfs/app/reaction_timing.py"
      via: "should_replan_immediately() triggers horizon_planner.plan() when deviations won't self-correct"
      pattern: "should_replan_immediately.*plan"
---
...
**8. Decision loop — Compute slot-0 costs (shared by steps 8a, 10, 11):**

Compute slot-0 costs BEFORE any learner if-blocks so the variables are available to all consumers. This prevents NameError when one learner is None but another needs the values.

```python
# Shared slot-0 cost computation — used by SeasonalLearner, Comparator, and RL learning
...
**8a. Decision loop — SeasonalLearner update (uses shared slot-0 costs):**
```python
...
**9. Decision loop — ReactionTimingTracker update and re-plan trigger:**
```python
if reaction_timing is not None and plan is not None:
    ...
    # LERN-03: Use learned threshold to decide whether to re-plan immediately
    if plan_action_str != actual_action_str and reaction_timing.should_replan_immediately():
        ...
        try:
            plan = horizon_planner.plan(..., confidence_factors=confidence_factors)
            lp_action = plan.slots[0].action if plan and plan.slots else lp_action
        except Exception as e:
            log("warning", f"Immediate re-plan failed: {e}")
```

**10. Decision loop — Comparator residual update (uses shared slot-0 costs from step 8):**
```python
...
**11. Decision loop — RL learning step (uses shared slot-0 costs from step 8):**
```python
if rl_agent is not None and not override_active:
    if plan_slot0_cost is not None and actual_slot0_cost is not None:
...
# Check reaction timing re-plan trigger (LERN-03)
assert 'should_replan_immediately' in src, 'Missing should_replan_immediately() call in main.py'
...
10. reaction_timing.should_replan_immediately() triggers horizon_planner.plan() when deviation detected and threshold says it won't self-correct (LERN-03 completion)
11. Comparator.compare_residual() called with slot-0 costs
</verification>
